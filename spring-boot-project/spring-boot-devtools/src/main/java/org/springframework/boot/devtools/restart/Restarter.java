...
public class Restarter {
	...
	protected Restarter(Thread thread, String[] args, boolean forceReferenceCleanup, RestartInitializer initializer) {
		...
		this.initialUrls = initializer.getInitialUrls(thread);//如果不是主线程或者不能开启devtools则返回null
		this.mainClassName = getMainClassName(thread);//当前Main方法Class
		this.applicationClassLoader = thread.getContextClassLoader();//当前线程的类加载器
		this.args = args;
		this.exceptionHandler = thread.getUncaughtExceptionHandler();//异常处理器
		this.leakSafeThreads.add(new LeakSafeThread());//添加一个较早创建的线程，以便不保留RestartClassLoader，并加入到线程池中
	}
	...
	protected void initialize(boolean restartOnInitialize) {
		preInitializeLeakyClasses();//CGlib有关
		if (this.initialUrls != null) {//之前Restart实例化时得到的url
			this.urls.addAll(Arrays.asList(this.initialUrls));//urls中也放一份
			if (restartOnInitialize) {//如果采用自动重启
				this.logger.debug("Immediately restarting application");
				immediateRestart();//启动线程
			}
		}
	}
	...
	private void immediateRestart() {
		...
		SilentExitExceptionHandler.exitCurrentThread();//当前线程退出 TODO
	}
	...
	private void preInitializeLeakyClasses() {
		try {
			Class<?> readerClass = ClassNameReader.class;//CGLIB中的一个类
	...
	public static void initialize(String[] args, boolean forceReferenceCleanup, RestartInitializer initializer,
		...
		synchronized (INSTANCE_MONITOR) {
			if (instance == null) {//instance是Restarter内部的static对象（Restarter类型），如果其他线程没有（或者之前）设置instance
				localInstance = new Restarter(Thread.currentThread(), args, forceReferenceCleanup, initializer);//创建新的Restarter
				instance = localInstance;
			}
		}
		if (localInstance != null) {//Restarter创建成功了
	...
	private class LeakSafeThread extends Thread {
		...
		<V> V callAndWait(Callable<V> callable) {
			...
			try {
				join();//主进程堵塞，子进程执行
				return (V) this.result;//目前调试得到的该结果为null
			}
			catch (InterruptedException ex) {
				Thread.currentThread().interrupt();//抛出异常当前线程中断
				throw new IllegalStateException(ex);
			}
		}
